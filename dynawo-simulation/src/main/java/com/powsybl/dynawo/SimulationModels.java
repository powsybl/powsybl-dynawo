 ...
 */
public class SimulationModels implements DydDataSupplier {
    ...
    public static SimulationModels createFrom(DynawoSimulationContext context, List<BlackBoxModel> dynamicModels,
        ...
        for (BlackBoxModel bbm : dynamicModels) {
            ...
            bbm.createDynamicModelParameters(context.getDynamicModelsParameters()::add);
        }
        ParametersSet networkParameters = context.getDynawoSimulationParameters().getNetworkParameters();
        for (BlackBoxModel bbem : eventModels) {
            bbem.createMacroConnections(adder);
            bbem.createDynamicModelParameters(context.getDynamicModelsParameters()::add);

...
public class PhaseShifterIAutomationSystemBuilder extends AbstractPhaseShifterModelBuilder<PhaseShifterIAutomationSystemBuilder> {
    ...
    public static PhaseShifterIAutomationSystemBuilder of(Network network, String modelName) {
        return of(network, modelName, ReportNode.NO_OP);
    }

    public static PhaseShifterIAutomationSystemBuilder of(Network network, String modelName, ReportNode reportNode) {
        ModelConfig modelConfig = MODEL_CONFIGS.getModelConfig(modelName);
        if (modelConfig == null) {
            BuilderReports.reportModelNotFound(reportNode, PhaseShifterIAutomationSystemBuilder.class.getSimpleName(), modelName);
            return null;
        }
    ...
    public PhaseShifterIAutomationSystem build() {
        return isInstantiable() ? new PhaseShifterIAutomationSystem(dynamicModelId, transformer.getEquipment(), parameterSetId, modelConfig) : null;
    }

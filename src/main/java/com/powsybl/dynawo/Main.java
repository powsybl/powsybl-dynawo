...
public final class Main {
    ...
    public static void main(String[] args) {
        ...
        try (ComputationManager computationManager = new LocalComputationManager(LocalComputationConfig.load())) {
            provider.run(network, dynamicModelsSupplier, EventModelsSupplier.empty(), curvesSupplier, network.getVariantManager().getWorkingVariantId(), computationManager, parameters).join();
        } catch (IOException e) {

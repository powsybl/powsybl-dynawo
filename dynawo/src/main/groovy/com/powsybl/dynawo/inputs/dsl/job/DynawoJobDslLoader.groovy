 ...
 */
package com.powsybl.dynawo.inputs.dsl.job
...
class DynawoJobDslLoader {
    ...
    static void loadDsl(Binding binding, Network network, Consumer<Job> consumer, DynawoDslLoaderObserver observer) {
        ...
        binding.job = { String name, Closure<Void> closure ->
            ...
            // create solver
            Solver solver = new Solver(jobSpec.solverSpec.lib, jobSpec.solverSpec.file, jobSpec.solverSpec.id)
            // create modeler
            Modeler modeler = new Modeler(jobSpec.modelerSpec.compile, jobSpec.modelerSpec.preCompiledModelsDir, jobSpec.modelerSpec.useStandardModelsPreCompiledModels, jobSpec.modelerSpec.modelicaModelsDir, jobSpec.modelerSpec.useStandardModelsModelicaModels, jobSpec.modelerSpec.iidm, jobSpec.modelerSpec.parameters, jobSpec.modelerSpec.parameterId, , jobSpec.modelerSpec.dyd, jobSpec.modelerSpec.initialState)
            // create simulation
            Simulation simulation = new Simulation(jobSpec.simulationSpec.startTime, jobSpec.simulationSpec.stopTime, jobSpec.simulationSpec.activeCriteria, jobSpec.simulationSpec.precision)
            // create outputs
            Outputs outputs = new Outputs(jobSpec.outputsSpec.directory, jobSpec.outputsSpec.curve)
            outputs.addAppenders(appenders)
            // create job
            Job job = new Job(name, solver, modeler, simulation, outputs)
            consumer.accept(job)

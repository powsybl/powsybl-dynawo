package com.powsybl.dynawo.simulator.csi;
 ...
 */
public class DynawoSimulatorCSITest {
    ...
    public void testCSI() throws Exception {
        try (FileSystem fs = Jimfs.newFileSystem(Configuration.unix())) {
            ...
            DynawoExporter exporter = DynawoExporterFactory.findDefault().create(platformConfig);
            DynawoResults result = tester.simulate(network, provider, exporter, platformConfig);
            LOGGER.debug("metrics " + result.getMetrics().get("success"));
            ...
            assertEquals(new Double(0.911176), result.getTimeSeries().get(new Double(30.0)).get(index));
        }
    }
    ...
    public void testGroovy() throws Exception {
        try (FileSystem fs = Jimfs.newFileSystem(Configuration.unix())) {
            ...
            // check final voltage of bus close to the event
            int index = result.getNames().indexOf("NETWORK__N1011____TN_Upu_value");
            assertEquals(new Double(0.931558), result.getTimeSeries().get(new Double(30.0)).get(index));
        }
    }
    ...
    private DynawoProvider configureProvider(Network network) {
        ...
        parameters
            .add(new DynawoParameter("capacitor_no_reclosing_delay", DynawoParameterType.DOUBLE.getValue(), "300"));
        parameters.add(new DynawoParameter("dangling_line_currentLimit_maxTimeOperation",
            DynawoParameterType.DOUBLE.getValue(), "90"));
        parameters.add(
            new DynawoParameter("line_currentLimit_maxTimeOperation", DynawoParameterType.DOUBLE.getValue(), "90"));
        ...
        parameters.add(new DynawoParameter("transformer_currentLimit_maxTimeOperation",
            DynawoParameterType.DOUBLE.getValue(), "90"));
        ...
        parameters.add(
            new DynawoParameter("transformer_tolV", DynawoParameterType.DOUBLE.getValue(), "0.014999999700000001"));
        ...
        parameters
            .add(new DynawoParameter("event_disconnectExtremity", DynawoParameterType.BOOLEAN.getValue(), "true"));
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DynawoSimulatorCSITest.class);
}

...
class EventActivePowerVariationGroovyExtension extends AbstractPureDynamicGroovyExtension<EventModel> implements EventModelGroovyExtension {
    ...
    EventActivePowerVariationGroovyExtension() {
        modelTags = [EventActivePowerVariationBuilder.TAG]
    }

    @Override
    protected EventActivePowerVariationBuilder createBuilder(Network network, Reporter reporter) {
        new EventActivePowerVariationBuilder(network, reporter)
    }
}
